...
class SCA(nn.Module):
    def __init__(self,in_channel,norm_groups=32):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=in_channel,out_channels=in_channel,kernel_size=3,padding=1,stride=1)
        self.n_head = 1
    ...
    def forward(self,x1,x2):
        ...
        query2, key2, value2 = qkv1.chunk(3, dim=2)  # bhdyx
...
class UNet(nn.Module):
    def __init__(
        ...
        self.use_sca = sca
        if self.use_sca:
            self.sca = SCA(inner_channel * channel_mults[-1])
        ...
        self.final_conv = Block(pre_channel, pre_channel, groups=norm_groups)
        self.final_conv1 = nn.Conv2d(pre_channel,1,1)

    def forward(self, x, time, xl=None):
        # if xl:
        #     print(xl.shape)
        ...
        if self.use_sca:
            ...
            return self.final_conv1(self.final_conv(x))
        else:
            # pp=[]
            # for i in range(len(feats)):
            #     pp.append(0)
            return self.final_conv1(self.final_conv(x)),fea,None
